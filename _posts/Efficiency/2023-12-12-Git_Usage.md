...
2. 提交（Commit）：每次对代码进行修改并保存后，都可以创建一个提交。提交包含了修改的内容、提交者的信息以及提交时间戳。
3. 分支（Branch）：分支是Git中用于隔离不同开发线的一种机制。每个分支都可以独立地开发，最后再合并到主分支上。
4. 合并（Merge）：合并是将两个分支的代码合并到一起的操作。在合并时，Git会尝试自动合并代码，如果存在冲突，则需要手动解决。
5. 工作区（Workspace）：工作区是开发者在本地机器上存放待处理的文件的地方。
6. 暂存区（Staging Area）：暂存区是用于存放已经修改但还未提交的文件的地方。开发者可以使用`git add`命令将文件添加到暂存区。

### 一些术语

- 克隆：克隆远程仓库的代码到本地
- 下拉：从远程仓库拉取最新的代码
- 合并：目的是解决冲突，将本地的代码与远程仓库中的代码进行合并
- 提交：这里指的是commit，指在本地创建一个提交，用于保存一次修改的记录
- 上推：将本地的代码及记录推送到远程仓库
